...
function imageProcessor(filename) {
    ...
    const monochromeDestination = uploadPathResolver('monochrome-'.concat(filename));
       ...
       if(isMainThread()) {
        try {
            ...
            resizeWorker.on('exit', (code) => {
                if(code !==0) {
                    reject(new Error('Exited with status code '.concat(code)));
                }
            })
            //monochromeWorker Events

            monochromeWorker.on('message', (message)=> {
                monochromeWorkerFinished = true;
                if(resizedWorkerFinished = true) {
